...
namespace de4dot.code.deobfuscators.CryptoObfuscator {
	public class DeobfuscatorInfo : DeobfuscatorInfoBase {
		...
		const string DEFAULT_REGEX = @"^(KSP|IO|PQ|PQS|LED|DAE|OBJ|MPE|XOR|OR|CCD|LAN|CQS|CQ|UT|ID|CD|TA)(?:`\d+)?$&^(get_|set_|add_|remove_)?[A-Z]{2,3}(?:`\d+)?$&!^(get_|set_|add_|remove_)?c[0-9a-f]{32}(?:`\d+)?$&" + DeobfuscatorBase.DEFAULT_ASIAN_VALID_NAME_REGEX;
